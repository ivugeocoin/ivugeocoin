...
#if defined(NDEBUG)
# error "IvugeoCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "IvugeoCoin Signed Message:\n";
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees)
{
   int64_t nSubsidy = 7.6 * COIN;
    ...
    nSubsidy >>= (nHeight / 5256000); // IvugeoCoin: 5,256mil blocks in ~10 years
...
bool IsInitialBlockDownload()
{
    ...
    if (lockIBDState)
        return false;
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 || pindexBestHeader->GetBlockTime() < GetTime() - 24 * 60 * 60);
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            string txHashIn = txin.prevout.hash.ToString();

            if (nSpendHeight > 271990) {
              if (txHashIn == "c27b2f4842442d0073b8dd6c903416666c6f7924c9350fda73a3d34b0c037aa4" || txHashIn == "e6296b75ee4fb639938019868f7e0c6c21ea695f6e67d90530b6bfe3170372d8") {
                return state.DoS(100, error("CTransaction::CheckTransaction() : hacked coins"));
              }
            }
...
void ThreadScriptCheck() {
    RenameThread("ivugeocoin-scriptch");
